...
class DetailProductVM(val repository: ProductRepository,val repoCart : CartRepository) : ViewModel() {
    ...
    fun getById(id : Int)  {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                if(repository.fetchProducts().isEmpty()){
                    productState = null
                }
                productState = repository.fetchProducts().find { it.id == id }
            }
        }
    }
